...
extern const char show_secrets_mrenclave[];
extern const char show_secrets_mrsigner[];

struct provision_entry provision_entries[2] = {
    {"redis-server", redis_server_mrenclave, redis_server_mrsigner, "intelsgxrocks!"},
    {"show-secrets", show_secrets_mrenclave, show_secrets_mrsigner, "42istheanswer!"}
};
...
int main( void )
{
    ...
    ret = mbedtls_x509_crt_parse_file(&srvcert, "secret-provisioning-service-crt.pem");
    assert(ret == 0);
    ret = mbedtls_pk_parse_keyfile(&pkey, "secret-provisioning-service-key.pem", NULL);
    assert(ret == 0);
...
reset:
    ...
    struct provision_entry* matching_entry = NULL;
    for (unsigned int i = 0;
         i < sizeof(provision_entries) / sizeof(struct provision_entry); ++i) {
        struct provision_entry* e = &provision_entries[i];
        
        if (memcmp(body->mr_enclave.m, e->mrenclave, sizeof(body->mr_enclave.m)) == 0 &&
            memcmp(body->mr_signer.m, e->mrsigner, sizeof(body->mr_signer.m)) == 0) {
            matching_entry = e;
            break;
        }
    }
    ...
    if (matching_entry) {
        mbedtls_printf( " ... success. Matched MRENCLAVE/MRSIGNER for %s.\n", matching_entry->appname);
    } else {
        mbedtls_printf(" ... failed. No matching MRENCLAVE/MRSIGNER found!\n");
        goto reset;
    }
     ...
     */
    mbedtls_printf( "  > Sending secret to %s ...", matching_entry->appname );
    fflush( stdout );

    const int32_t secret_len = strlen(matching_entry->secret) + 1;

    assert((int) sizeof(secret_len) == mbedtls_ssl_write(&ssl, (unsigned char*) &secret_len,
                                                         sizeof(secret_len)));
    assert(secret_len == mbedtls_ssl_write(&ssl, (unsigned char*) matching_entry->secret,
                                           secret_len));
    mbedtls_printf( " done\n" );
    fflush(stdout);
